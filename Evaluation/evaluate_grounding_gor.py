...
parser.add_argument('--img_folder_path', type=str, default="/data3/xuyiy/datasets/ifashion/new_data/processed_images")
...
def main():
    ...
    if args.mode == "valid":
        ...
        gor_candidates = np.load("/data3/xuyiy/datasets/ifashion/new_data/map/cate_iid_dict.npy", allow_pickle=True).item()
    else:
        history = np.load(os.path.join(args.data_path, "processed", "test_history_clipembs.npy"), allow_pickle=True).item()
        gor_candidates = np.load("/data3/xuyiy/datasets/ifashion/new_data/map/cate_iid_dict.npy", allow_pickle=True).item()
        test_grd = np.load("/data3/xuyiy/datasets/ifashion/new_data/data_ready/test_grd.npy", allow_pickle=True).item()
        fitb_test_dict = np.load("/data3/xuyiy/datasets/ifashion/new_data/data_ready/fitb_test_dict.npy", allow_pickle=True).item()

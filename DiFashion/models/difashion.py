...
class DiFashion(ModelMixin, ConfigMixin):
    ...
    def forward(self, batch, img_dataset, history, null_img, mask_ratio, coupling_mask_ratio, cate_mask_ratio, weight_dtype, generator):
        ...
        if self.args.use_mutual_guidance:
            ...
            mutual_cond = self.fashion_encoder(mutual_cond)   # 把这里的clamp去掉了
        else:
    ...
    def pred_ori_sample_given_epsilon(self, timestep, noisy_latent, epsilon):
        ...
        return pred_ori_sample.clamp(-1., 1.)  # TODO: 已修改
    ...
    def fashion_generation(
        ...
        bsz = olists.shape[0]
        olen = olists.shape[1]
        fill_idx = torch.nonzero(olists == 0)  # 找到需要生成的image索引
        ...
        for i,cate in enumerate(fill_cate):
            uid = uids[fill_idx[i][0]].item()  # fill_ids[i][0]表明当前填的这个空属于该batch中的第几套outfit
        ...
        for i,_ in enumerate(olists):
            ...
            indicies = torch.arange(olen) + i * olen  # 对应flat_outift的image索引
            indicies[gen_mask] = -torch.arange(all_num, all_num + gen_num) - 1  # 把需要生成的indices改为对应的第几个生成的img, 即fill_idx中的位置-1
